...
int function dressIndex(int Stage, int Slot)
	return ((PapyrusUtil.ClampInt(Stage, 1, Stages) - 1) * 1) + Slot
endfunction
...
bool function PositionDress(bool Output, int Position, int Stage)
	int i = dressIndex(Stage, 0)
	bool[] dress = dressArray(Position)	
	Output = dress[i]
...
String function PositionOrgasmType(String Output, int Position, int Stage)
	...
	String[] orgasmType = orgasmTypeArray(Position)	
	Output = orgasmType[i]
	return Output
endFunction
...
String function PositionSfxVoice(String Output, int Position, int Stage)
	...
	String[] sfxVoiceType = sfxVoiceTypeArray(Position)	
	Output = sfxVoiceType[i]
	return Output
endFunction
...
String function PositionSfxSound(String Output, int Position, int Stage)
	...
	String[] sfxSounds = sfxSoundArray(Position)	
	Output = sfxSounds[i]
	return Output
endFunction
...
function AddPositionStage(int Position, string AnimationEvent, float forward = 0.0, float side = 0.0, float up = 0.0, float rotate = 0.0, bool silent = false, bool openmouth = false, bool strapon = true, int sos = 0, bool dress = false, String orgasmType = "", String sfxVoiceType = "", String sfxSounds = "")
	...
	if Position == 0
		...
		; Offset stage overflow
		if (eid + kOrgasmTypeEnd) >= orgasmType0.Length
		...
		; Offset stage overflow
		if (uid + kDressEnd) >= dress0.Length
			Log("WARNING: Offsets position overflow, resizing! - Current offsets: "+dress0, "AddPositionStage("+Position+", "+AnimationEvent+")")
			dress0 = PapyrusUtil.ResizeBoolArray(dress0, (dress0.Length + 32))
		endIf		
	endIf
	...
	; Save undress
	bool[] _dress = dressArray(Position)
	_dress[uid + 0] = dress
	uid += kDressEnd

	; Save sfx voice
	String[] _orgasmType = orgasmTypeArray(Position)
	_orgasmType[eid + 0] = orgasmType
	eid += kOrgasmTypeEnd
...
function Initialize()
	...
	dress0  = Utility.CreateBoolArray(0)
	dress1  = Utility.CreateBoolArray(0)
	dress2  = Utility.CreateBoolArray(0)
	dress3  = Utility.CreateBoolArray(0)
	dress4  = Utility.CreateBoolArray(0)

	orgasmType0 = Utility.CreateStringArray(0)
	orgasmType1 = Utility.CreateStringArray(0)
	orgasmType2 = Utility.CreateStringArray(0)
	orgasmType3 = Utility.CreateStringArray(0)
	orgasmType4 = Utility.CreateStringArray(0)

	sfxVoice0 = Utility.CreateStringArray(0)
	sfxVoice1 = Utility.CreateStringArray(0)
	sfxVoice2 = Utility.CreateStringArray(0)
	sfxVoice3 = Utility.CreateStringArray(0)
	sfxVoice4 = Utility.CreateStringArray(0)

	sfxSounds0 = Utility.CreateStringArray(0)
	sfxSounds1 = Utility.CreateStringArray(0)
	sfxSounds2 = Utility.CreateStringArray(0)
	sfxSounds3 = Utility.CreateStringArray(0)
	sfxSounds4 = Utility.CreateStringArray(0)
...
bool[] function dressArray(int Position)
	if Position == 0
		return dress0
	elseIf Position == 1
		return dress1
...
int property kOrgasmTypeEnd hidden
	int function get()
		return 1
	endFunction
endProperty
...
string[] function orgasmTypeArray(int Position)
	if Position == 0
		return orgasmType0
	elseIf Position == 1
		return orgasmType1
...
int property kOrgasm    = 6 autoreadonly hidden
...
function InitArrays(int Position)
	...
	elseIf Position == 1
		...
		dress1 = Utility.CreateBoolArray((Stages * kDressEnd))
		orgasmType1 = Utility.CreateStringArray((Stages * kOrgasmTypeEnd))
	...
	elseIf Position == 2
		...
		dress2 = Utility.CreateBoolArray((Stages * kDressEnd))
		orgasmType2 = Utility.CreateStringArray((Stages * kOrgasmTypeEnd))
	...
	elseIf Position == 3
		...
		dress3 = Utility.CreateBoolArray((Stages * kDressEnd))
		orgasmType3 = Utility.CreateStringArray((Stages * kOrgasmTypeEnd))
	...
	elseIf Position == 4
		...
		dress4 = Utility.CreateBoolArray((Stages * kDressEnd))
		orgasmType4 = Utility.CreateStringArray((Stages * kOrgasmTypeEnd))
...
function ExportJSON()
	...
	while Position < PositionCount
		...
		while stg < StageCount
			...
			; JsonUtil.SetPathIntValue(Filename, Path+".flag.orgasm["+stg+"]", Flags[(fi + 6)])
