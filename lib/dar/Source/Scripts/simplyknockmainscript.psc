...
GlobalVariable  OpeningDoors = None
...
if (! OpeningDoors)
OpeningDoors = Game.GetFormFromFile(0x0A042525, "ImmersiveInteractions.esp") as GlobalVariable
endif

if OpeningDoors && OpeningDoors.GetValue() == 1 

If PlayerRef.IsSneaking() == True && PlayerRef.IsWeaponDrawn()
akDoor.Activate(PlayerRef)

elseif PlayerRef.IsSneaking()
; Nothing, my mod takes over
...
function KnockOnDoor(ObjectReference akDoor)
	...
	Game.ForceThirdPerson()
	Game.DisablePlayerControls(false, false, true, false, false, false, false)	
	Debug.SendanimationEvent(PlayerRef, "AO_IdleKnock")
	utility.wait(1.1)
